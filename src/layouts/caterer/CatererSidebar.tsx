...
const NAV: NavItem[] = [
  ...
  { id: 'banking',          label: 'Banks & Banking Information', icon: <CreditCard       size={16} strokeWidth={1.8} /> },
  { id: 'establishments',   label: 'My Clients / Establishments', icon: <MapPin           size={16} strokeWidth={1.8} /> },
...
const CATERER_PATHS: Record<CatererNavId, string> = {
  ...
  profil:             '/caterer/profil',
  banking:            '/caterer/banking',
  establishments:     '/caterer/establishments',
  menus:              '/caterer/menus',
  'document-vault':   '/caterer/document-vault',
  contracts:          '/caterer/contracts',
  modules:            '/caterer/modules',
  corrections:        '/caterer/corrections',
  'go-live':          '/caterer/go-live',
  ecoloop:            '/caterer/ecoloop',
}
...
export interface CatererSidebarProps {
  ...
  drawerOpen: boolean
  onDrawerClose: () => void
  /** Pixels below the 52px header — non-zero only while the Support Access Session banner is showing. */
  topOffset?: number
}

/* ── Shared nav content ─────────────────────────────────── */
function NavContent({ collapsed, onClose }: { collapsed: boolean; onClose?: () => void }) {
  const { logout } = useCatererAuth()
      ...
      <div className="px-3 pb-3 shrink-0" style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 12 }}>
        {collapsed ? (
          ...
          >
            ...
            <span className="flex-1 text-[13px] font-medium leading-snug">Log out</span>
          </button>
        )}
      </div>
    </>
...
/* ── CatererSidebar ───────────────────────────────────────── */
export function CatererSidebar({ isDesktop, collapsed, onToggle, drawerOpen, onDrawerClose, topOffset = 0 }: CatererSidebarProps) {

  /* ── Desktop: fixed sidebar ── */
  if (isDesktop) {
    return (
      <aside
        className="fixed left-0 bottom-0 flex flex-col overflow-hidden"
        style={{
          top: 52 + topOffset,
