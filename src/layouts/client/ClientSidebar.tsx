...
interface NavItem { id: ClientNavId; label: string; icon: ReactNode; progress?: number }
...
function NavContent({ collapsed, onClose }: { collapsed: boolean; onClose?: () => void }) {
  const { t } = useLang()
  const NAV_LABELS: Record<ClientNavId, string> = {
      ...
      <nav className="flex-1 px-3 pt-3 pb-2 overflow-y-auto overflow-x-hidden">
        ...
        <div className="space-y-1">
          {NAV.map(item => {
            if (collapsed) {
              return (
                <div key={item.id} className="flex justify-center">
                  <NavLink to={CLIENT_PATHS[item.id]} title={NAV_LABELS[item.id] ?? item.label}
            ...
            return (
              <NavLink key={item.id} to={CLIENT_PATHS[item.id]}
