...
/* ── Types ─────────────────────────────────────────────── */
type Status   = 'pre-onboarding' | 'en-cours' | 'soumis' | 'corrections' | 'approuves' | 'go-live'
type Vertical = 'Schools' | 'Daycares' | 'Camps' | 'CSS'
  ...
  return (
    <div className="relative rounded-2xl px-5 pt-5 pb-4 overflow-hidden"
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span className="text-[11px] font-medium px-2 py-0.5 rounded-lg" style={{ background: color+'12', color }}>
          {sub}
        </span>
      </div>
      <div className="text-[38px] font-black tabular-nums leading-none tracking-tighter" style={{ color }}>{value}</div>
  ...
  return (
    <div className="p-4 lg:p-7 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-7">
        <StatCard value={stats.total}   label="Total Caterers"    sub="all active"    color="#60a5fa"  icon={<Users         size={16} strokeWidth={1.8} />} />
        <StatCard value={stats.inProg}  label="In Progress"       sub="onboarding"    color="#a78bfa"  icon={<Calendar      size={16} strokeWidth={1.8} />} />
        <StatCard value={stats.blocked} label="Needs Review"      sub="action needed" color="#f87171"  icon={<AlertTriangle size={15} strokeWidth={1.8} />} />
        <StatCard value={stats.goLive}  label="Ready for Go-Live" sub="approved"      color="#a3e635"  icon={<CheckCircle2  size={16} strokeWidth={1.8} />} />
      </div>
